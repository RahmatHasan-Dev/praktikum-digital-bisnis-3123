    ...
    <div class="p-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            <div>
                ...
                <p class="text-slate-500 mt-2 font-medium">Gunakan form search untuk menemukan kategori berdasarkan nama.</p>
            </div>
            ...
            <form method="GET" action="{{ route('admin.categories.index') }}" class="flex gap-3 items-center">
                <input
                    ...
                    class="w-full md:w-72 px-5 py-3 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium"
                />
                ...
                <tbody class="divide-y border-t">
                    @forelse($categories as $index => $category)
                        <tr class="hover:bg-slate-50/50 transition">
                            <td class="px-4 py-6 font-bold text-slate-400">{{ $categories->firstItem() + $index }}</td>
                            <td class="px-4 py-6">{{ $category->id }}</td>
                            <td class="px-4 py-6">
                                <p class="font-extrabold text-slate-900">{{ $category->name }}</p>
                                <p class="text-xs text-slate-500">Created: {{ $category->created_at->format('d-m-Y') }}</p>
                            </td>
                            <td class="px-4 py-6">
                                <span class="inline-block px-3 py-1 rounded-xl bg-white border border-slate-200 text-slate-600 text-sm font-bold">{{ $category->slug }}</span>
                                <p class="text-xs text-slate-500">Updated: {{ $category->updated_at->format('d-m-Y') }}</p>
                            </td>
                            <td class="px-4 py-6">
                                <div class="flex gap-2">
                                    <a href="{{ route('admin.categories.edit', $category->id) }}" class="p-2.5 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-600 hover:text-white transition" aria-label="Edit">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-10 text-center text-slate-500 font-medium">Kategori tidak ditemukan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
